...
def process_uploaded_file(uploaded_file, predictor):
    try:
        ...
        if feature_count != expected_features:
            if feature_count > expected_features:
                st.info(f"Note: Your data has {feature_count} features, but the model expects {expected_features}. The extra features will be ignored.")
            else:
                st.info(f"Note: Your data has {feature_count} features, but the model expects {expected_features}. Missing features will be filled with zeros.")
        ...
        try:
            ...
            st.info("Please ensure your data has the correct format: numeric values only, with each row representing a sample.")
            return None
    except Exception as e:
    ...
    if page == "Single Prediction":
            ...
            if uploaded_file is not None:
                try:
                    ...
                    if data.empty:
                        ...
                        st.warning("Warning: The uploaded file contains multiple samples. Only the first sample will be used.")
                    ...
                    if 'sample_values' in locals():
                        ...
                        if len(sample_values) != expected_features:
                            if len(sample_values) > expected_features:
                                st.info(f"Note: Your data has {len(sample_values)} features, but the model expects {expected_features}. The extra features will be ignored.")
                            else:
                                st.info(f"Note: Your data has {len(sample_values)} features, but the model expects {expected_features}. Missing features will be filled with zeros.")
        ...
        if uploaded_file is not None:
            try:
                ...
                non_numeric_cols = data.select_dtypes(exclude=['number']).columns.tolist()
                if non_numeric_cols:
                    st.info(f"Non-numeric columns detected: {', '.join(non_numeric_cols)}. These will be excluded from numerical analysis.")
                ...
                try:
                    ...
                    if 'missing_values' in quality_metrics and quality_metrics['missing_values'] > 0:
                        st.warning(f"⚠️ Your data contains {quality_metrics['missing_values']} missing values ({quality_metrics['missing_percentage']:.2f}%).")
                    
                    if 'negative_values' in quality_metrics and quality_metrics['negative_values'] > 0:
                        st.warning(f"⚠️ Your data contains {quality_metrics['negative_values']} negative values, which may be unusual for reflectance data.")
                    
                    if 'potential_outliers' in quality_metrics and quality_metrics['potential_outliers'] > 0:
                        st.warning(f"⚠️ Detected {quality_metrics['potential_outliers']} potential outliers in your data.")
                    
                    # Spectral curves visualization
                    st.subheader("Spectral Curves")
                    st.write("This plot shows the average spectral curve across all samples, with the shaded area representing one standard deviation.")
                    ...
                    if not numeric_data.empty:
                        if numeric_data.shape[1] > 5:
                            ...
                            for col in sample_cols:
                                ax.plot(numeric_data[col].values[:20], label=str(col))  # Plot first 20 values
                            ax.legend()
                        ...
                        if numeric_data.shape[1] <= 20:
                            ...
                            sns.heatmap(corr, cmap='coolwarm', ax=ax, vmin=-1, vmax=1, center=0)
