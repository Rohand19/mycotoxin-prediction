...
class DONPredictor:
    ...
    def build(self):
        ...
        optimizer = tf.keras.optimizers.Adam(
            learning_rate=learning_rate_schedule, clipnorm=1.0, beta_1=0.9, beta_2=0.999, epsilon=1e-07
        )

        # Compile with MSE like reference
        model.compile(
            optimizer=optimizer, loss="mean_squared_error", metrics=["mae", tf.keras.metrics.RootMeanSquaredError()]
        )
