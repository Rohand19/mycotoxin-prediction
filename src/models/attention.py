...
class MultiHeadSelfAttention(Layer):
    ...
    def build(self, input_shape):
        ...
        self.query_dense = tf.keras.layers.Dense(
            self.output_dim, use_bias=self.use_bias, name='query'
        )
        self.key_dense = tf.keras.layers.Dense(
            self.output_dim, use_bias=self.use_bias, name='key'
        )
        self.value_dense = tf.keras.layers.Dense(
            self.output_dim, use_bias=self.use_bias, name='value'
        )
        
        # Output projection
        self.combine_heads = tf.keras.layers.Dense(
            self.input_dim, use_bias=self.use_bias, name='output'
        )
